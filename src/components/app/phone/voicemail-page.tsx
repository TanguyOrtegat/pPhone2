...
const VoicemailPage: React.FC = () => {
    ...
    const onVoicemailClicked = (id: number) => {
        if (state.editMode) {
            SetState({ ...state, voicemails: state.voicemails.map(mail => mail.id == id ? { ...mail, selected: !mail.selected } : mail) });
        } else {
            SetState({ ...state, playerMailId: state.playerMailId ? undefined : id, voicemails: state.voicemails.map(mail => { return { ...mail, read: mail.id == id || mail.read } }) });
        }
    };
    ...
    const getVoiceMails = () => {
        return state.voicemails.map(mail => <Voicemail key={mail.id} {...mail} editMode={state.editMode}
    ...
    return (
        <div id="voicemail-page">
            <div className={`header ${state.playerMailId != undefined ? "header-disable" : ""}`}>
                ...
                <div className="header-right" onClick={() => state.playerMailId == undefined && toggleEditMode()}>
                    {state.editMode ? "Done" : "Edit"}
                </div>
            </div>
            <div className="page-container">
                <ul id="voicemail-list" className={`list-view ${state.playerMailId != undefined ? "has-player" : ""}`}>
                    <CSSTransitionGroup transitionName="voicemail-transition" transitionEnter={false} transitionLeaveTimeout={300} >
                        {getVoiceMails()}
                    </CSSTransitionGroup>
                </ul>
            </div>
...
const Voicemail: React.FC<IVoicemailProps> = (props: IVoicemailProps) => {
    return (
        <li onClick={props.onVoicemailClicked}
            ...
            <div className="voicemail-container">
                ...
                {props.showPlayer && <VoiceMailPlayer
                    onDeleteClicked={props.onDeleteVoicemail} length={props.length} />}
            </div>
        </li >
    );
}
    ...
    return (
        <React.Fragment>
            <div className="player-line">
                <div className="player-line-progress"
